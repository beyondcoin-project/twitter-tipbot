...
async function main(){
  ...
  for (let i in accs){
    try {
      ...
      const balanceFromOld = await bynd.getBalance(`twttr-${accs[i]}`);
      if (balanceFromOld !== 0) {
        let res = await bynd.move(
          `twttr-${accs[i]}`,
          id(usr),
          Number(balanceFromOld)
        );
        // If move is successful, log it!
        if (res) logger.info(`Transferred ${balanceFromOld} BYND from twttr-${accs[i]} to ${id(usr)}!`);
      }
    }catch(e){
...
// Get a list of all twitter accounts on beyondcoind.
async function getAccounts(){
  let accs = await bynd.listAccounts();
...
async function getAddress(userId) {
  try {
    ...
    await bynd.getNewAddress(userId);
  } catch (e) {
