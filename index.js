...
const bynd = new Client({
  username: config.get("beyondcoind.username"),
  password: config.get("beyondcoind.password"),
  port: config.get("beyondcoind.port")
});
...
logger.info("Started Beyondcoin Twitter tipbot.");
...
// ADD terms
  await T.post("statuses/update", {
    ...
    "Under no circumstances shall the Beyondcoin Project or Beyondcoin Developers be held responsible for lost, stolen or misdirected funds.",
    in_reply_to_status_id: tweet.id_str
  });
}
async function doBalance(tweet, msg) {
  try {
    const balance = await bynd.getBalance(id(tweet.user.id_str), config.get("bot.requiredConfirms")); // Amount of confirms before we can use it.
    const post = await T.post("statuses/update", {
      in_reply_to_status_id: tweet.id_str,
      status: `@${tweet.user.screen_name} You have ${balance} BYND.`
    });
    logger.info(
...
async function doWithdraw(tweet, msg) {
  ...
  let txId = await bynd.sendFrom(id(tweet.user.id_str), address, amount);
  await T.post("statuses/update", {
    status: `@${tweet.user.screen_name} You withdrew ${amount} BYND to ${address}. \n${txLink(txId)}`,
    in_reply_to_status_id: tweet.id_str
  });
  logger.info(
    `User ${
      tweet.user.screen_name
    } withdraw ${amount} BYND to ${address}, tweet id: ${tweet.id_str}`
  );
  } catch (e) {
    logger.error(e);
  }
...
async function doTip(tweet, msg) {
  try {
    ...
    const balanceFromUser = await bynd.getBalance(id(tweet.user.id_str), config.get("bot.requiredConfirms"));
    if (balanceFromUser < amount) {
      return await T.post("statuses/update", {
        status: `@${tweet.user.screen_name} You tried tipping more than you have! You are ${amount-balanceFromUser} BYND short.`,
        in_reply_to_status_id: tweet.id_str
      });
    }
    const txId = await bynd.sendFrom(id(tweet.user.id_str), tipToAddress, Number(amount), 1);
    await T.post("statuses/update", {
      status: `@${tweet.user.screen_name} Tipped ${amount} BYND! We'd say who it was to, but then Twitter would ban the bot. \nTransaction: ${txLink(txId)}`,
      in_reply_to_status_id: tweet.id_str
    });
    logger.info(
      `@${tweet.user.screen_name}(${tweet.user.id_str}) tipped ${
        msg[2]
      }(${userToTip}) ${amount} BYND.`
    );
  } catch (e) {
    logger.error(e);
  }
...
async function getAddress(userId) {
  try {
    let uAddresses = await bynd.getAddressesByAccount(userId);
    if (uAddresses.length > 0) return uAddresses[0];
    let nAddress = await bynd.getNewAddress(userId);
    return nAddress;
  } catch (e) {
    logger.error(e);
  }
...
function getValidatedAmount(amount) {
  amount = amount.trim();
  if (amount.toLowerCase().endsWith("bynd")) {
